...
// 8x8 点阵 ASCII 字体库（每个字符用8字节表示，每字节8位对应一行像素）
// 索引：字符的 ASCII 码（32-126 为可打印字符）
// 规则：bit=1 表示该位置有像素，bit=0 表示无像素
static const uint8_t ascii_8x8[95][8] = {
    ...
    {0x00, 0x00, 0x07, 0x04, 0x00, 0x00, 0x00, 0x00}, // 39: '
    ...
    {0x00, 0x32, 0x49, 0x59, 0x51, 0x3e, 0x00, 0x00}, // 64: @
    ...
    {0x00, 0x02, 0x04, 0x08, 0x10, 0x20, 0x00, 0x00}, // 92: \ (反斜杠)
...
// 获取字符的点阵数据（返回8字节数组的指针，若字符不支持则返回NULL）
static inline const uint8_t* ascii_8x8_get(char c) {
    uint8_t idx = (uint8_t)c;
    if (idx < 32 || idx > 126) {
        return NULL; // 只支持可打印ASCII字符
    }
    return ascii_8x8[idx - 32]; // 索引偏移（32是第一个可打印字符）
}
